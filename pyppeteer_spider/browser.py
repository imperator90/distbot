...
class ManagedBrowser:
    ...
    def __init__(self, browser: pyppeteer.browser.Browser):
...
class BrowserManager():
    ...
    def __init__(self, page_manager: PageManager,
                 ...
                 browser_memory_limit: Optional[int] = None, # Limit browser's memory. Units: mb
                 default_viewport: Optional[Dict[str, int]] = None): # Set custom viewport size. Default is full page.
        log_save_path = Path(log_file_path) if log_file_path else Path(__file__).parent.joinpath('logs/browser_manager.log')
        ...
        self.proxy_addr_gen = self.cyclic(proxy_addr) # If multiple proxy addresses are provided, they will be used cyclicly as browsers are created.
        self.user_data_dir_gen = self.cyclic(user_data_dir) # If multiple chrome profiles are provided, they will be used cyclicly as browsers are created.
    ...
    async def get_browser(self) -> pyppeteer.browser.Browser:
        ...
        await self.page_manager.add_browser_page_s(browser, self.browser_page_count - 1)
    ...
    async def remove_browser(self, browser: pyppeteer.browser.Browser) -> None:
    ...
    def replacing_browser(self, browser: pyppeteer.browser.Browser) -> bool:
    ...
    async def replace_browser(self, browser: pyppeteer.browser.Browser) -> None:
        """Close a browser and launch a new one."""
        # Make sure browser has not already been replaced.
        managed_browser = self.managed_browsers.get(browser) # returns None if browser has been replaced.
        if managed_browser: 
    ...
    async def browser_error(self, browser: pyppeteer.browser.Browser,
                            error: bool) -> None:
    ...
    async def shutdown(self) -> None:
        """Close all browsers."""
        await asyncio.gather(*[self.remove_browser(browser)
            for browser in list(self.managed_browsers.keys())
        ])
