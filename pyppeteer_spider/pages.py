...
class PageManager:
    ...
    def __init__(self,
                 ...
                 default_nav_timeout: Optional[int] = None, # Change Pyppeteer's default maximum navigation timeout. (Units: ms, default is 30000ms)
                 blocked_urls: Optional[List[str]] = None, # URL patterns to block. Wildcards ('*') are allowed.
                 js_injection_scripts: Optional[List[str]] = None, # JavaScript functions that will be invoked whenever the page is navigated.
        ...
        log_save_path = Path(log_file_path) if log_file_path else Path(__file__).parent.joinpath('logs/page_manager.log')
        self.logger = get_logger("PageManager", log_save_path=log_save_path, log_level=log_level)
    ...
    async def get_page(self) -> Tuple[bool, pyppeteer.page.Page]:
        ...
        self.logger.debug(f"({self}) Got idle page in {round(time()-wait_start,2)}s")
    ...
    async def prep_page__(self, page: pyppeteer.page.Page) -> pyppeteer.page.Page:
    ...
    async def set_idle(self, page_s_brow: Union[pyppeteer.page.Page,
                                                List[pyppeteer.page.Page],
                                                pyppeteer.browser.Browser]) -> None:
    ...
    async def remove_page_s(self, page_s_brow: Union[pyppeteer.page.Page,
                                                List[pyppeteer.page.Page],
                                                pyppeteer.browser.Browser]) -> None:
    ...
    async def add_browser_page_s(self, browser: pyppeteer.browser.Browser,
                                page_count: int) -> List[pyppeteer.page.Page]:
        ...
        if page_count > 0:
            ...
            self.logger.info(f"Finished initializing {page_count} browser page(s).")
            return new_pages

    async def add_page_settings(self, page_s_brow: Union[pyppeteer.page.Page,
                                                List[pyppeteer.page.Page],
                                                pyppeteer.browser.Browser]) -> None:
    ...
    async def add_page_settings_(self, page: pyppeteer.page.Page) -> None:
        ...
        for f in Path(__file__).parent.joinpath('automation_detection').glob(
                "*.js"):
            self.logger.info(f"(page {page}) Adding automation detection prevention script: {f.name}")
        ...
        if self.headless:
            for f in Path(__file__).parent.joinpath('headless_detection').glob(
                    "*.js"):
                self.logger.info(f"(page {page}) Adding headless detection prevention script: {f.name}")
                tasks.append(page.evaluateOnNewDocument(f.read_text()))
    ...
    async def to_pages_(self, page_s_brow: Union[pyppeteer.page.Page,
                                                List[pyppeteer.page.Page],
                                                pyppeteer.browser.Browser]) -> None:
        """Convert argument to a list of pages."""
        if isinstance(page_s_brow, pyppeteer.page.Page):
            return [page_s_brow]
        if isinstance(page_s_brow, pyppeteer.browser.Browser):
            return await page_s_brow.pages()
        if isinstance(page_s_brow, (list, tuple)):
            return page_s_brow
        raise ValueError(
            "Argument should be in (pyppeteer.page.Page, pyppeteer.browser.Browser, list, tuple)"
        )

    async def close_page(self, page: pyppeteer.page.Page) -> None:
